    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="hidden md:block overflow-y-auto max-h-[calc(100vh-250px)] rounded-lg shadow border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (!empty($data['admins'])): ?>
                        <?php foreach ($data['admins'] as $admin): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?= htmlspecialchars($admin['gender']) ?></td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <?php
                                    $statusText = $admin['is_active'] ? 'Active' : 'Inactive';
                                    $statusClass = $admin['is_active'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
                                    ?>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <button class="view-details-button bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md text-xs transition duration-300"
                                            ...
                                            data-department="<?= htmlspecialchars($admin['department'] ?? '') ?>"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500">No admin users found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="md:hidden space-y-4 overflow-y-auto max-h-[calc(100vh-250px)]">
            <?php if (!empty($data['admins'])): ?>
                <?php foreach ($data['admins'] as $admin): ?>
                    <div class="bg-gray-50 p-4 rounded-lg shadow border border-gray-200">
                        <div class="flex justify-between items-center mb-2">
                            <h3 class="text-lg font-bold text-gray-800"><?= htmlspecialchars($admin['name']) ?></h3>
                            <?php
                            $statusText = $admin['is_active'] ? 'Active' : 'Inactive';
                            $statusClass = $admin['is_active'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
                            ?>
                        ...
                        <p class="text-sm text-gray-700 mb-1"><strong>ID:</strong> <?= htmlspecialchars($admin['id']) ?></p>
                        <p class="text-sm text-gray-700 mb-1"><strong>Email:</strong> <?= htmlspecialchars($admin['email']) ?></p>
                        <p class="text-sm text-gray-700 mb-1"><strong>Department:</strong> <?= htmlspecialchars($admin['department'] ?? '') ?></p>
                        <p class="text-sm text-gray-700 mb-4"><strong>Gender:</strong> <?= htmlspecialchars($admin['gender']) ?></p>
                        <div class="flex space-x-2">
                            <button class="view-details-button bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-md text-xs transition duration-300"
                                data-id="<?= htmlspecialchars($admin['id']) ?>"
                                data-name="<?= htmlspecialchars($admin['name']) ?>"
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p class="text-center py-4 text-gray-500">No admin users found.</p>
            <?php endif; ?>
        </div>
    </div>
</main>
...
<div id="viewModal" class="fixed inset-0 z-50 bg-black bg-opacity-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-96 max-w-full">
        <h2 class="text-xl font-bold mb-4">Admin Details</h2>
...
<div id="editModal" class="fixed inset-0 z-50 bg-black bg-opacity-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-lg p-6 w-[500px] max-w-full">
        <h2 class="text-xl font-bold mb-4 text-gray-800 flex items-center gap-2">
            <i class="fas fa-user-edit text-yellow-500"></i> Edit Admin
        </h2>
...
<style>
    ...
    /* Global Modal Styles (as provided previously, ensuring consistency) */
    .modal {
        position: fixed;
        top: 0;
        left: 0;
...
<script>
    ...
    document.getElementById('editForm').addEventListener('submit', function(e) {
        ...
        // Ensure this action URL is correct for editing an admin
        this.action = "<?php echo URLROOT; ?>/admin/editadminlist/" + id; // Changed to editAdmin
    });
    ...
    deleteButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Ensure this action URL is correct for deleting an admin
            document.getElementById('deleteForm').action = "<?php echo URLROOT; ?>/admin/deleteadminlist/" + id; // Changed to deleteAdmin
